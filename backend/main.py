...
def predict_weather(request: RequestData):
    ...
    probs_climatology, _ = climatology_probabilities(df, target_date, request.historical_years_used)
    ...
    # 3) ML features + predictions
    built = build_features_for_date(df, target_date)
    if built is None:
        probs_ml = {k + "_percent": 0.0 for k in ["very_hot", "very_cold", "very_wet", "very_windy", "very_uncomfortable"]}
        rain_pred = {"rain_mm_predicted": 0.0, "rain_probability_percent": 0.0}
    else:
        X, _ = built
        probs_ml = ml_predict(X)
        rain_pred = rain_predict(X)
    ...
    for key in ["very_wet", "very_hot", "very_cold", "very_windy", "very_uncomfortable"]:
        ...
        p_fore = float(probs_forecast.get(clim_key, 0.0))
        p_ml = float(probs_ml.get(clim_key, 0.0))
        weighted_sum = p_ml * WEIGHTS["ml"] + p_clim * WEIGHTS["clim"] + p_fore * WEIGHTS["fore"]
        weight_total = WEIGHTS["ml"] + WEIGHTS["clim"] + WEIGHTS["fore"]
